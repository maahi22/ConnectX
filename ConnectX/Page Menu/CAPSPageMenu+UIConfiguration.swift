...
extension CAPSPageMenu {
    ...
    func setUpUserInterface() {
        let viewsDictionary = ["menuScrollView":menuScrollView, "controllerScrollView":controllerScrollView, "btnCancel":btnCancel]
        ...
        let menuScrollView_constraint_H:Array = NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[menuScrollView]-45-|", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: viewsDictionary)
       ...
       // menuScrollView.backgroundColor = .yellow
       // controllerScrollView.backgroundColor = .magenta
        
        
        
        //Mahendra Added for customisation
        
       // btnCancel.frame = CGRect(x: 0 , y: 0.0, width: 40, height: configuration.menuHeight)//CGRect(x:  (self.view.frame.width - 50) , y: 0, width: 45, height: 40)
        //btnCancel.titleLabel?.text = "X"
        let image = UIImage(named: "backText.png")
        btnCancel.setImage(image, for: .normal)//(image, for: .normal)
        btnCancel.backgroundColor = .white
        self.view.addSubview(btnCancel)
    
        btnCancel.translatesAutoresizingMaskIntoConstraints = false
        
        view.addConstraint(NSLayoutConstraint(item: btnCancel, attribute: .top, relatedBy: .equal, toItem: self.topLayoutGuide, attribute: .bottom, multiplier: 1, constant: 0))
        ...
        view.addConstraint(NSLayoutConstraint(item: btnCancel, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute,multiplier: 1, constant: 45))
        view.addConstraint(NSLayoutConstraint(item: btnCancel, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute,multiplier: 1, constant: 30))
        view.addConstraint(NSLayoutConstraint(item: btnCancel, attribute: .trailingMargin, relatedBy: .equal, toItem: view, attribute: .trailingMargin, multiplier: 1, constant: +10))
